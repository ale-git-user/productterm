...
public class Description implements RF2Constants {
	...
	public static void fillFromRf2(Description d, String[] lineItems) {
		...
		d.setEffectiveTime(lineItems[DES_IDX_EFFECTIVETIME].equals("")?null:lineItems[DES_IDX_EFFECTIVETIME]);
	...
	public void addAcceptability(LangRefsetEntry lang)  {
		if (lang.isActive()) {
	...
	public Boolean isReleased() {
		...
		if (released == null) {
			return !(effectiveTime == null || effectiveTime.equals(""));
		}
		return released;
	}
	...
	public Acceptability getAcceptability(String langRefsetId) {
		//Are we working with the JSON map, or RF2 Lang refset entries?
		if (acceptabilityMap != null) {
			for (Map.Entry<String, Acceptability> entry: acceptabilityMap.entrySet()) {
